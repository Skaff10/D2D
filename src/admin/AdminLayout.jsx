      ...
      >
        <div className="flex flex-col h-full overflow-y-auto">
          ...
          <nav className="flex-1 p-4 space-y-1">
            {sidebarLinks.map((link) => (
              <NavLink
                ...
                onClick={() => setSidebarOpen(false)}
                className={({ isActive }) =>
                  `flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all ${isActive ? "bg-primary/10 text-primary" : "text-text-secondary hover:text-white hover:bg-white/5"}`
                }
              >
                <link.icon size={18} />
                {link.name}
          ...
          <div className="p-4 border-t border-border-warm">
            <div className="text-xs text-text-muted mb-3 px-4 truncate">
              {user?.email}
            </div>
            <button
              onClick={handleLogout}
              className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-red-400 hover:bg-red-400/10 transition-all w-full"
            >
              <LogOut size={18} />
              Sign Out
            </button>
